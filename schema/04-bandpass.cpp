...
class StaticFilter final : public ATK::ModellerFilter<double>
{
    ...
    ATK::StaticCapacitor<DataType> c11{2.2e-09};
    ATK::StaticResistor<DataType> r8{220000};
    ATK::StaticResistorCapacitor<DataType> r7c12{40000, 2.2e-07};
  ...
  public:
    ...
    gsl::index get_nb_components() const override { return 4; }
    ...
    gsl::index get_number_parameters() const override { return 0; }
    ...
    {
        Eigen::Matrix<DataType, 2, 2> jacobian(
            Eigen::Matrix<DataType, 2, 2>::Zero());
        auto jac0_0 = 0 - (steady_state ? 0 : c11.get_gradient()) -
                      r8.get_gradient() -
                      (steady_state ? 0 : r7c12.get_gradient());
        auto jac0_1 =
            0 + (steady_state ? 0 : c11.get_gradient()) + r8.get_gradient();
        auto jac1_0 = 0 + -1;
        auto jac1_1 = 0;
    ...
    {
        // update_steady_state
        c11.update_steady_state(1. / input_sampling_rate, dynamic_state[0],
                                dynamic_state[1]);
        r7c12.update_steady_state(1. / input_sampling_rate, static_state[0],
                                  dynamic_state[0]);
    ...
    void process_impl(gsl::index size) const override
    {
        for (gsl::index i = 0; i < size; ++i)
        {
            ...
            // Update state
            c11.update_state(dynamic_state[0], dynamic_state[1]);
            r7c12.update_state(static_state[0], dynamic_state[0]);
    ...
    template <bool steady_state> bool iterate() const
    {
        ...
        Eigen::Matrix<DataType, 2, 1> eqs(
            Eigen::Matrix<DataType, 2, 1>::Zero());
        auto eq0 = +(steady_state ? 0 : c11.get_current(d0_, d1_)) +
                   r8.get_current(d0_, d1_) -
                   (steady_state ? 0 : r7c12.get_current(s0_, d0_));
