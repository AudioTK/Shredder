...
//==============================================================================
BShredderAudioProcessor::BShredderAudioProcessor()
  :
...
#  endif
  ...
  , highPassFilter(BShredder::createStaticFilter_stage1())
  , oversamplingFilter(1)
  , preDistortionToneShapingFilter(BShredder::createStaticFilter_stage2())
  , bandPassFilter(BShredder::createStaticFilter_stage3())
  , distLevelFilter(BShredder::createStaticFilter_stage4())
  , distFilter(BShredder::createStaticFilter_stage5())
  , postDistortionToneShapingFilter(BShredder::createStaticFilter_stage6())
  , lowpassFilter(1)
  , decimationFilter(1)
  , DCFilter(1)
...
BShredderAudioProcessor::~BShredderAudioProcessor() = default;

//==============================================================================
const juce::String BShredderAudioProcessor::getName() const
{
  return JucePlugin_Name;
}

bool BShredderAudioProcessor::acceptsMidi() const
{
...
bool BShredderAudioProcessor::producesMidi() const
{
...
bool BShredderAudioProcessor::isMidiEffect() const
{
#if JucePlugin_IsMidiEffect
  return true;
#else
...
double BShredderAudioProcessor::getTailLengthSeconds() const
{
  return 0.0;
}

int BShredderAudioProcessor::getNumPrograms()
{
  return 2;
}

int BShredderAudioProcessor::getCurrentProgram()
{
  return lastParameterSet;
}

void BShredderAudioProcessor::setCurrentProgram(int index)
{
  if(index != lastParameterSet)
  {
    ...
    {
      const char* preset0
          = "<BShredder><PARAM id=\"distLevel\" value=\"0\" /><PARAM id=\"lowLevel\" value=\"0\" /><PARAM id=\"highLevel\" "
            "value=\"0\" /> <PARAM id=\"midLevel\" value=\"0\" /><PARAM id=\"midFreq\" value=\"1000\" /></BShredder>";
    ...
    {
      const char* preset1 = "<BShredder><PARAM id=\"distLevel\" value=\"100\" /><PARAM id=\"lowLevel\" value=\"20\" /><PARAM "
                            "id=\"highLevel\" value=\"20\" /> <PARAM id=\"midLevel\" value=\"15\" /><PARAM "
                            "id=\"midFreq\" value=\"1000\" /></BShredder>";
...
const juce::String BShredderAudioProcessor::getProgramName(int index)
{
  if(index == 0)
  {
...
void BShredderAudioProcessor::changeProgramName(int index, const juce::String& newName)
{
}

//==============================================================================
void BShredderAudioProcessor::prepareToPlay(double dbSampleRate, int samplesPerBlock)
{
...
void BShredderAudioProcessor::releaseResources()
{
...
#ifndef JucePlugin_PreferredChannelConfigurations
bool BShredderAudioProcessor::isBusesLayoutSupported(const juce::BusesLayout& layouts) const
{
#  if JucePlugin_IsMidiEffect
...
void BShredderAudioProcessor::processBlock(juce::AudioSampleBuffer& buffer, juce::MidiBuffer& midiMessages)
{
...
//==============================================================================
bool BShredderAudioProcessor::hasEditor() const
{
  return true; // (change this to false if you choose to not supply an editor)
}

juce::AudioProcessorEditor* BShredderAudioProcessor::createEditor()
{
  return new BShredderAudioProcessorEditor(*this, parameters);
}

//==============================================================================
void BShredderAudioProcessor::getStateInformation(juce::MemoryBlock& destData)
{
...
void BShredderAudioProcessor::setStateInformation(const void* data, int sizeInBytes)
{
...
{
  return new BShredderAudioProcessor();
}
